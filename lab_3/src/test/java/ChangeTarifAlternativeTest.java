...
public class ChangeTarifAlternativeTest {
    ...
    public void changetarif() {
        for (WebDriver driver : drivers) {
            ...
            driver.findElement(By.xpath("//li[15]/a/div[2]")).click();

            driver.findElement(By.xpath("//button[@id='save']/span")).click();
            WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(10));
            Alert alert = wait.until(ExpectedConditions.alertIsPresent());
            String alertText = alert.getText();
            assertTrue(alertText.contains("Превышено количество сайтов для смены тарифа"), "Ожидался alert с текстом 'Превышено количество сайтов для смены тарифа'");
            try {
                Thread.sleep(2000);
